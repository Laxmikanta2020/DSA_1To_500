...
//Find youngest employee
       ...
       var reduce2 = employees.stream().map(Emp::getAge).reduce(Integer::min);
...
//Find oldest employee per department
        Map<String, Optional<Emp>> collect20 = employees.stream().collect(Collectors.groupingBy(Emp::getDep, Collectors.maxBy(Comparator.comparing(Emp::getAge))));
