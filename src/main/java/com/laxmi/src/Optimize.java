...
public class Optimize {
    ...
    public static void main(String[] args) {
        ...
        List<Emp> itEmployees = employees.stream()
                .filter(emp -> "IT".equals(emp.getDep()))
                .toList();

// Alternative: Using method reference with predicate
        List<Emp> itEmployees2 = employees.stream()
                .filter(emp -> emp.getDep().equals("IT"))
                .collect(Collectors.toList());

// 2. Find employees whose age > 30
        List<Emp> ageAbove30 = employees.stream()
                .filter(emp -> emp.getAge() > 30)
                .toList();

// 3. Find employees from Bengaluru AND IT
        List<Emp> bengaluruIT = employees.stream()
                .filter(emp -> "Bengaluru".equals(emp.getLocation())
                        && "IT".equals(emp.getDep()))
                .toList();

// 4. Find employees not from HR
        List<Emp> nonHREmployees = employees.stream()
                .filter(emp -> !"HR".equals(emp.getDep()))
                .toList();
...
// 5. Sort employees by location then name
        List<Emp> sortedByLocationThenName = employees.stream()
                .sorted(Comparator.comparing(Emp::getLocation)
                        .thenComparing(Emp::getName))
                .toList();
// 1. Get first 3 employees
        List<Emp> firstThree = employees.stream()
                .limit(3)
                .toList();

// 2. Skip first 2 employees
        List<Emp> skipFirstTwo = employees.stream()
                .skip(2)
                .toList();
...
// 2. Parallel reduction with custom combiner
        class ComplexAccumulator {
            ...
            public double getMean() { return sumAge / count; }
